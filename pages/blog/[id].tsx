  ...
  return (
    <Layout>
      <article className="max-w-7xl  mx-auto px-10  my-10 ">
        ...
        {/* Título */}
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 text-center text-gray-700">
          {data.titulo}
        </h1>
        ...
        {/* Resumen */}
        <p className="text-base sm:text-lg text-gray-700 mb-8 whitespace-pre-line">
          {data.resumen}
        </p>
          ...
          <section key={seccion.id} className="mb-10">
            ...
            {seccion.bloques.map((bloque) => {
                    ...
                    {partes.map((parte, i) => {
                      if (parte.match(/^\[.*\]$/)) {
                        ...
                        return (
                          <h3
                            key={i}
                            className="md:text-xl sm:text-base  text-gray-600 mb-2"
                          >
                            {textoSinCorchetes}
                          </h3>
                        );
                        ...
                        return (
                          parte.trim() !== "" && (
                            <p
                              key={i}
                              className="text-base sm:text-lg text-gray-700 whitespace-pre-line pb-2"
                            >
                              {parte}
                            </p>
                          )
              ...
              if (bloque.tipo === "LISTA") {
                ...
                if (h3Match) {
                  partes.push(
                    <h3
                      key="lista-h3"
                      className="text-base sm:text-lg font-semibold mb-2 text-blue-700"
                    >
                      {h3Match[1].trim()}
                    </h3>
                  );
                ...
                return (
                  <div key={bloque.id} className="mb-6">
                    ...
                    {bracketText && (
                      <p className="pt-3 text-sm sm:text-base text-gray-700 whitespace-pre-line">
                        {bracketText}
                      </p>
                    )}
                  </div>
              ...
              if (bloque.tipo === "IMAGEN") {
                return (
                  <div key={bloque.id} className=" w-80  px-1 md:w-full overflow-x-auto">
                    <SyntaxHighlighter
                      language={detectarLenguaje(bloque.contenido)}
                      style={oneDark}
