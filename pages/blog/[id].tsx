...
export default function BlogDetalle() {
  ...
  return (
    <div>
      ...
      <article className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-6 my-10">
      {/* Título */}
      <h1 className="text-3xl md:text-4xl font-extrabold mb-4 text-gray-900">{data.titulo}</h1>
      {/* Imagen principal */}
      <img
        src={data.imagen1}
        alt={"ww"}
      ...
      {data.seccionesTexto.map((seccion) => (
        <section key={seccion.id} className="mb-10">
          {/* Subtítulo */}
          <h2 className="text-2xl font-bold text-amber-600 mb-5">{seccion.subtitulo}</h2>
          ...
          {seccion.bloques.map((bloque) => {
            ...
            if (bloque.tipo === "TEXTO") {
              return (
                <p key={bloque.id} className="text-gray-800 text-base mb-4 whitespace-pre-line">
                  {bloque.contenido}
                </p>
              );
            }
            ...
            if (bloque.tipo === "LISTA") {
              ...
              return (
                <div key={bloque.id} className="mb-6">
                  ...
                  {items.length > 0 && (
                    <ol className="list-decimal ml-6 text-base text-gray-800 space-y-1 mb-2">
                      {items.map((item, idx) => (
                  ...
                  {bracketText && (
                    <p className="bg-gray-100 p-3 rounded text-sm text-gray-700 mt-2 shadow whitespace-pre-line">
                      {bracketText}
                    </p>
                  )}
                </div>
